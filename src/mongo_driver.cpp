...
{
    LOG(trace) << " from col:" << col << " doc:" << doc;
    std::lock_guard<std::mutex> lo(db_mutex);
    try{
...
{
    LOG(trace) << " in col:" << col << " filter:" << filter;
    std::lock_guard<std::mutex> lo(db_mutex);
    try{
